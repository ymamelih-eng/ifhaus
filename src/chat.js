...
import { getRelevantKnowledge, refreshKnowledge, getModelNames } from './knowledge.js';
import { extractPhone, saveLead } from './lead.js';
import {
  checkScope,
  CLASSIFIER_SYSTEM,
...
// Shared chatbot engine used by the site API (/api/chat) and the Meta adapter.

// Lead source written to Google Sheets, per channel.
export const SOURCE_LABELS = { web: 'Site', instagram: 'Instagram', messenger: 'Facebook' };
...
/**
 * Processes one user message and returns the /api/chat response body.
 * Returns { error } (HTTP 400 on the site API) for an empty message; throws on AI failure.
 * @param {object} input
 * @param {string} input.message
 * @param {string} [input.sessionId]
 * @param {'web'|'instagram'|'messenger'} [input.source]
 * @param {string} [input.instagramUsername]
 * @param {string} [input.senderId]  channel user id (Meta PSID / IGSID), stored with the lead
 */
export async function handleChat({ message, sessionId, source = 'web', instagramUsername = '', senderId = '' } = {}) {
  if (!message?.trim()) return { error: 'message required' };
    ...
    if (verdict === 'OUT_OF_SCOPE') return outOfScopeResponse(sid, session);
    ...
    if (text.includes(OUT_OF_SCOPE_TOKEN)) return outOfScopeResponse(sid, session);
    ...
    if (phone && !session.leadSaved) {
      try {
        leadResult = await saveLead({
          ...
          ...(senderId ? { channelUserId: senderId } : {})
        });
  ...
  return { sessionId: sid, reply, leadSaved: Boolean(phone && session.leadSaved), leadResult, cta };
}
